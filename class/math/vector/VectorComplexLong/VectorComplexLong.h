// file: $isip/class/math/vector/VectorComplexLong/VectorComplexLong.h
// version: $Id: VectorComplexLong.h 10640 2007-01-27 02:36:04Z tm334 $
//

// make sure definitions are only made once
//
#ifndef ISIP_VECTOR_COMPLEX_LONG
#define ISIP_VECTOR_COMPLEX_LONG
...
// VectorComplexLong: this is a vector of ComplexLong objects, which
// inherits the MVector template class.
//
class VectorComplexLong : public MVector<ComplexLong, complexlong> {
...
public:
  ...
  bool8 debug(const unichar* message) const {
    return MVector<ComplexLong, complexlong>::debug(name(), message);
  }
  ...
  //
  VectorComplexLong(int32 length = DEF_LENGTH,
		    complexlong value = DEF_VALUE) {
    assign(length, value);
  }
  ...
  bool8 read(Sof& sof, int32 tag, const String& name = CLASS_NAME) {
    return MVector<ComplexLong, complexlong>::read(sof, tag, name);
  }
  
  // method: write
  //
  bool8 write(Sof& sof, int32 tag, const String& name = CLASS_NAME) {
    return MVector<ComplexLong, complexlong>::write(sof, tag, name);
  }    
...
public:
  ...
  //
  VectorComplexLong(const String& arg, unichar delim = DEF_DELIM) {
    assign(arg, delim);
  }
  ...
  //
  VectorComplexLong(const unichar* arg, unichar delim = DEF_DELIM) {
    assign(arg, delim);
  }
  
  // method: operator=
  //  this method assigns a long value to the vector
  //
...
public:
  ...
  // method: real
  //
  bool8 real(VectorLong& arg) const {
  ...
  // method: imag
  //
  bool8 imag(VectorLong& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = v_d[i].imag();
    }
  ...
  // method: abs
  //
  bool8 abs(VectorLong& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = (int32)v_d[i].mag();
    }
    return true;
  }
  ...
  // method: mag
  //
  bool8 mag(VectorLong& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = (int32)v_d[i].mag();
    }
  ...
  // method: angle
  //
  bool8 angle(VectorLong& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = (int32)v_d[i].angle();
    }
    return true;
  }
  ...
  // method: conjugate
  //
  bool8 conjugate(const VectorComplexLong& arg) {
...
private:
  ...
  //  
  bool8 operator %(const VectorComplexLong& vector);
  bool8 operator %=(const VectorComplexLong& vector);
  bool8 band();
  bool8 bor();
  bool8 bxor();
