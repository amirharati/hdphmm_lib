// file: $isip/class/vector/VectorByte/vbyt_02.cc
// version: $Id: vbyt_02.cc 10636 2007-01-26 22:18:09Z tm334 $
//
...
//
bool8 VectorByte::diagnose(Integral::DEBUG level_a) {
  ...
  //
  MVector<Byte, byte8>::diagnose(level_a);
  ...
  //
  VectorByte val0((byte8)5);
  val0.assign(L"1, 3, 5, 7, 9");
  
  VectorByte val1(val0);
  if (val0.ne(val1)) {
  ...
  if (!val2.read(text_file, (int32)10) || (val2.length() != 0)) {
    ...
    return Error::handle(name(), L"read-text", Error::TEST,
			 __FILE__, __LINE__);
  }
  val2.clear();
  ...
  if (!val2.read(bin_file, (int32)10) || (val2.length() != 0)) {
    val2.debug(L"length 0 vector");
    return Error::handle(name(), L"read-bin", Error::TEST,
			 __FILE__, __LINE__);
  }
  val2.clear();
  ...
  // test new and delete
  //
  VectorByte* dyn_val0 = new VectorByte((int32)5);             
  dyn_val0->assign(L"1, 3, 5, 7, 9");
  
  VectorByte* dyn_val1 = new VectorByte(*dyn_val0);
...
//       //
//       VectorByte* vectors = new VectorByte[j * 100](num_elem);
...
//       delete [] vectors;
  ...
  // unichar constructor
  //
  VectorByte temp_vect_2(L"5, 5, 5, 5, 5");
