// file: $isip/class/math/vector/VectorComplexDouble/VectorComplexDouble.h
// version: $Id: VectorComplexDouble.h 10636 2007-01-26 22:18:09Z tm334 $
//

// make sure definitions are only made once
//
#ifndef ISIP_VECTOR_COMPLEX_DOUBLE
#define ISIP_VECTOR_COMPLEX_DOUBLE
...
// VectorComplexDouble: this is a vector of ComplexDouble objects, which
// inherits the MVector template class.
//
class VectorComplexDouble : public MVector<ComplexDouble, complexdouble> {
...
public:
  ...
  bool8 debug(const unichar* message) const {
    return MVector<ComplexDouble, complexdouble>::debug(name(), &message[0]);
  }
  ...
  //
  VectorComplexDouble(int32 length = DEF_LENGTH,
		      complexdouble value = DEF_VALUE) {
    assign(length, value);
  }
  ...
  bool8 read(Sof& sof, int32 tag, const String& name = CLASS_NAME) {
    return MVector<ComplexDouble, complexdouble>::read(sof, tag, name);
  }
  
  // method: write
  //
  bool8 write(Sof& sof, int32 tag, const String& name = CLASS_NAME) {
    return MVector<ComplexDouble, complexdouble>::write(sof, tag, name);
  }    
...
public:
  ...
  //
  VectorComplexDouble(const String& arg, unichar delim = DEF_DELIM) {
    assign(arg, delim);
  }
  ...
  //
  VectorComplexDouble(const unichar* arg, unichar delim = DEF_DELIM) {
    assign(arg, delim);
  }
  
  // method: operator=
  //  this method assigns a double value to the vector
  //
...
public:
  ...
  // method: real
  //
  bool8 real(VectorDouble& arg) const {
  ...
  // method: imag
  //
  bool8 imag(VectorDouble& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = v_d[i].imag();
    }
  ...
  // method: abs
  //
  bool8 abs(VectorDouble& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = v_d[i].mag();
    }
  ...
  // method: mag
  //
  bool8 mag(VectorDouble& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = v_d[i].mag();
    }
  ...
  // method: angle
  //
  bool8 angle(VectorDouble& arg) const {
    arg.setLength(length_d);
    for (int32 i = (int32)length_d - 1; i >= 0; i--) {
      arg(i) = v_d[i].angle();
    }
  ...
  // method: conjugate
  //
  bool8 conjugate(const VectorComplexDouble& arg) {
...
private:
  ...
  //  
  bool8 operator %(const VectorComplexDouble& vector);
  bool8 operator %=(const VectorComplexDouble& vector);
  bool8 band();
  bool8 bor();
  bool8 bxor();
