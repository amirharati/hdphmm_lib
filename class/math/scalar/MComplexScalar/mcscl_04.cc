// file: $isip/class/math/scalar/MComplexScalar/mcscl_04.cc
// version: $Id: mcscl_04.cc 6807 2001-04-24 15:28:14Z peng $
//
...
//
// arguments:
//  const MComplexScalar<TIntegral, TSize>& this: (output) class operand
...
bool8 MComplexScalarMethods::write(const MComplexScalar<TIntegral, TSize>& this_a,
...
//
template<class TIntegral, class TSize>
bool8 MComplexScalarMethods::writeData(const MComplexScalar<TIntegral, TSize>& this_a,
  ...
  //
    ...
    //
    TSize val[] = {(TSize)this_a.value_d.real(), (TSize)this_a.value_d.imag()};

    // range check the datatype
    //
    if ((this_a.value_d.real() != val[0]) ||
	(this_a.value_d.imag() != val[1])) {
      return Error::handle(name(), L"writeData", Error::DATATYPE_RANGE,
                           __FILE__, __LINE__, Error::WARNING);
    }
    
    if (sof_a.write(val, sizeof(TSize), 2) != 2) {
      return false;
    }
  }
...
//
template bool8
MComplexScalarMethods::write<ISIP_TEMPLATE_TARGET> 
(const MComplexScalar<ISIP_TEMPLATE_TARGET>&, Sof&, int32, const String&);

template bool8
MComplexScalarMethods::writeData<ISIP_TEMPLATE_TARGET> 
(const MComplexScalar<ISIP_TEMPLATE_TARGET>&, Sof&, const String&);
